    ...
    <div class="flex justify-end space-x-8">
      {#each buttons as button}
        {#if button.link === '#projects'}
          <a href={button.link} on:click|preventDefault={scrollIntoView} class="no-underline hover:underline transition-all duration-300 ease-in-out text:lg sm:text-2xl text-center">{button.name}</a>
        {:else}
